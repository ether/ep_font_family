# Contributing to ep_font_family

Thanks for helping improve `ep_font_family` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_font_family` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_font_family from your local clone
pnpm run plugins i --path ../ep_font_family

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_font_family/` are picked up after a server restart.
...
* Etherpad version, ep_font_family version, browser + OS
* Steps to reproduce
